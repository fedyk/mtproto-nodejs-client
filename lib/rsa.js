...
import { bytesToHex, hexToBytes, bigIntToBytes, bytesToBigInt, SHA1, } from './common.js';
const publisKeys = [
    {
...
(function (RSA) {
    async function getPublisKeysByHex() {
        ...
        for (const publisKey of publisKeys) {
            const RSAPublicKey = new Serializer(function (ctx) {
    ...
    RSA.getPublisKeysByHex = getPublisKeysByHex;
    ...
    RSA.getRsaKeyByFingerprints = getRsaKeyByFingerprints;
    function encrypt(publicKey, bytes) {
        const encryptedBigInt = bytesToBigInt(bytes).modPow(bigInt(publicKey.exponent, 16), bigInt(publicKey.modulus, 16));
