...
export class RPC {
    ...
    async handlePQResponse(buffer) {
        ...
        const publicKey = await RSA.getRsaKeyByFingerprints(server_public_key_fingerprints);
    ...
    async handleDHParams(buffer) {
        ...
        this.tmpAesKey = concatBytes(SHA1(concatBytes(this.newNonce, this.serverNonce)), (SHA1(concatBytes(this.serverNonce, this.newNonce))).slice(0, 12));
        this.tmpAesIV = concatBytes((SHA1(concatBytes(this.serverNonce, this.newNonce))).slice(12, 20), SHA1(concatBytes(this.newNonce, this.newNonce)), this.newNonce.slice(0, 4));
        const decryptedData = new ModeOfOperationIGE(this.tmpAesKey, this.tmpAesIV).decrypt(encrypted_answer);
        ...
        if (!bytesIsEqual(innerDataHash, SHA1(decryptedData.slice(20, 20 + innerDeserializer.offset)))) {
            throw new Error('Invalid hash in DH params decrypted data');
        }
        await this.storage.set('timeOffset', Math.floor(Date.now() / 1000) - serverDHInnerData.server_time);
    ...
    verifyDhParams(g, dhPrime, gA) {
        ...
        if (gA.greaterOrEquals(dhPrime.minus(bigInt.one))) {
            throw new Error('Server_DH_inner_data.g_a incorrect: g_a >= dh_prime - 1');
        }
        ...
        if (gA.greaterOrEquals(dhPrime.minus(twoPow))) {
            throw new Error('Server_DH_inner_data.g_a incorrect: g_a >= dh_prime - 2^{2048-64}');
        }
    }
    async generateDH(retryId = 0) {
        ...
        const serverSalt = xorBytes(this.newNonce.slice(0, 8), this.serverNonce.slice(0, 8));
        await this.setStorageItem('authKey', bytesToBytesRaw(authKey));
        await this.setStorageItem('serverSalt', bytesToBytesRaw(serverSalt));
        this.authKeyAuxHash = bytesToBytesRaw((SHA1(authKey)).slice(0, 8));
        if (!this.g) {
        ...
        const encryptedData = new ModeOfOperationIGE(this.tmpAesKey, this.tmpAesIV).encrypt(concatBytes(innerDataHash, innerData, getRandomBytes(paddingLength)));
    ...
    async handleDHAnswer(buffer) {
        ...
        if (serverDHAnswer._ === 'mt_dh_gen_ok') {
            const hash = (SHA1(concatBytes(this.newNonce, [1], this.authKeyAuxHash))).slice(4, 20);
        ...
        if (serverDHAnswer._ === 'mt_dh_gen_retry') {
            const hash = (SHA1(concatBytes(this.newNonce, [2], this.authKeyAuxHash))).slice(4, 20);
        ...
        if (serverDHAnswer._ === 'mt_dh_gen_fail') {
            const hash = (SHA1(concatBytes(this.newNonce, [3], this.authKeyAuxHash))).slice(4, 20);
    ...
    async handleEncryptedMessage(buffer) {
        ...
        const computedMessageKey = SHA256(concatBytes(authKey.slice(96, 128), plaintextData)).slice(8, 24);
        ...
        if (length > plaintextData.length) {
            console.warn(`Length in message ${messageId} to exceed the plaintext length:`, `${length} > ${plaintextData.length}`);
            return;
        }
        if (length % 4 !== 0) {
            console.warn(`Length ${length} in message ${messageId} is not a multiple of four`);
            return;
        }
    ...
    getAESInstance(authKey, messageKey, isServer) {
        const x = isServer ? 8 : 0;
        const sha256a = SHA256(concatBytes(messageKey, authKey.slice(x, 36 + x)));
        const sha256b = SHA256(concatBytes(authKey.slice(40 + x, 76 + x), messageKey));
        const aesKey = concatBytes(sha256a.slice(0, 8), sha256b.slice(8, 24), sha256a.slice(24, 32));
        const aesIV = concatBytes(sha256b.slice(0, 8), sha256a.slice(8, 24), sha256b.slice(24, 32));
